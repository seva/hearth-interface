...
- **Operational Protocol (2026-02-13)**:
    ...
    - **RCA Learning Loop**: Memorize any generalized learning from RCAs going forward.
...
### Future Capability: Agent-to-Human Delegation
- **RentAHuman.ai** (launched 2026-02-02): Marketplace where AI agents hire humans for physical tasks ($5-$500/hr). MCP + REST API integration. Payment in stablecoins.
- **Relevance to Hearth:** Solves the "Code is Manager" gap — property inspections, notary visits, key pickups, on-site verification. Things smart contracts can't do.
- **Maturity:** Very early. Claims 70K registered humans but only ~83 visible profiles. Task completion unreliable. Monitor for improvement.
- **Action:** When the platform stabilizes, integrate as an MCP tool. VixeYult could autonomously delegate physical-world Hearth operations to hired humans.
- See also: [Human API](https://chainwire.org/2026/02/11/human-api-launches-as-first-platform-for-ai-agents-to-hire-humans/) — competing platform, same concept.

### Infra & Ops
- **Cost Profile:** Monitored via `poll-cost.ps1`. Significantly reduced with DeepSeek V3.2 ($0.25/$0.38/M) as primary.
- **Stack:** Local Windows 11 + Hardhat + Solidity + Python.
...
- **API Key Format (2026-02-14):**
    - **Basescan/Etherscan V2:** 34 chars (e.g., `ZMQMW...`).
    - **Mistake:** I hallucinated a "Human API" key. Corrected via `KEYS.md` and user confirmation.
    - **Rule:** If a key looks like Etherscan, treat it as Etherscan. Do not invent new services without evidence.
