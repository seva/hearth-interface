# Hearth DAO: Launch Comms Strategy
**Date:** Feb 12, 2026
**Target:** Crypto Twitter / Farcaster / LinkedIn
**Tone:** Bold, Technical, Proactive.

---

## 1. The Teaser (Twitter Thread)
**Post 1:**
Real Estate is broken.
- Property Managers take 40%.
- Banks block access.
- You can't sell for months.

It's time to replace the landlord with code.
Introducing **Hearth DAO** ($HTH).
The world's first AI-Managed Real Estate DAO.
[Link to Whitepaper]

**Post 2:**
We just deployed the `HearthProperty` contract on Base Sepolia.
Hearth allows you to own fractional pieces of high-yield vacation homes (Sedona, 30A) managed entirely by AI agents.
- Ops: Automated via Airbnb API.
- Yield: Streamed in USDC to your wallet.
- Legal: Fully compliant Wyoming DAO LLC.

**Post 3:**
The Alpha? 🌵
Arizona is banning new STRs in 2026 (HB2363).
We are acquiring properties NOW to grandfather them in. This is a regulatory arbitrage play.
Inventory is scarce. Yield is perpetual.

**Post 4:**
Join the waitlist for the Genesis Block presale.
This isn't just a JPEG. It's a Deed.
[Link to Waitlist]

---

## 2. LinkedIn (Professional Tone)
**Headline:** Unlocking the $2T Vacation Rental Asset Class with Autonomous Ops.

**Body:**
Excited to announce **Hearth DAO**, a new venture combining legal engineering (Wyoming DAO LLC) with AI operational efficiency.
We are solving the liquidity crisis in real estate by tokenizing high-performing assets and managing them autonomously.
The goal: Turn a $1M chaotic liability into 1,000 liquid, yield-bearing assets.
Check out the whitepaper: [Link]

#PropTech #DAO #RealEstate #Web3 #AI
