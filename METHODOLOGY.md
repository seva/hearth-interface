...
| `Documents/` | Legal and compliance artifacts |
| `protocol/` | Smart contracts and deployment scripts |
| `docs/` | Discovery artifacts — hard gates for dependent phases |
...
For Hearth: legal discovery (securities counsel review) gates Reg D offering. Smart contract audit gates mainnet deployment.
...
- **`Documents/`** — legal filings, entity docs, roadmap. Committed, permanent.
...
- Test files mirror source structure (`protocol/test/`)
...
- *Acceptable* — untestable path (deployment scripts, migration code). Document why.
...
- Wyoming DAO LLC Act (W.S. 17-31)
- SEC Reg D (506(b) or 506(c))
- Securities laws (HRTH tokens are securities)
- Wyoming annual report requirements (Feb 1 each year)

When legal uncertainty exists, escalate to securities counsel (issue #48) before proceeding.
